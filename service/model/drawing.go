...
const (
	//发起图纸拆分 PUT  https://api.bimface.com/files/{fileId}/split
	splitDrawingURI string = "/files/%d/split"

	//获取图纸拆分状态 GET https://api.bimface.com/data/v2/files/{fileId}/frames
	splitDrawingFramesURI string = "/data/v2/files/%d/frames"

	//获取图纸列表 GET https://api.bimface.com/data/v2/files/{fileId}/drawingsheets
	drawingsheetsURI string = "data/v2/files/%d/drawingsheets"
)

//---------------------------------------------------------------------

func (o *Service) splitDrawingURL(fileId int64) string {
	return fmt.Sprintf(o.Endpoint.APIHost+splitDrawingURI, fileId)
}

func (o *Service) splitDrawingFramesURL(fileId int64) string {
	return fmt.Sprintf(o.Endpoint.APIHost+splitDrawingFramesURI, fileId)
}

func (o *Service) drawingSheetsURL(fileId int64) string {
	return fmt.Sprintf(o.Endpoint.APIHost+drawingsheetsURI, fileId)
}
...
func (o *Service) doSplitDrawing(fileId int64, isGet bool) (*response.SplitDrawingStatus, error) {
	...
	var result *response.SplitDrawingStatus
...
func (o *Service) GetDrawingSheets(fileId int64) (map[string]interface{}, error) {
	accessToken, err := o.AccessTokenService.Get()
	if err != nil {
		return nil, err
	}
	...
	resp := o.ServiceClient.Get(o.drawingSheetsURL(fileId), headers.Header)
